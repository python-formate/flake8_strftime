...
if [ -z "$(git status --porcelain --untracked-files=no)" ] || [ "$1" == "-f" ]; then
  ...
  autopep8 --in-place --select "$errors" -a --recursive flake8_strftime/
  autopep8 --in-place --select "$belligerent" -a -a -a -a -a --recursive flake8_strftime/
